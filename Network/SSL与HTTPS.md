...
## SSL/TLS
为啥要发明 SSL 这个协议捏？
因为原先互联网上使用的 HTTP 协议是明文的，存在很多缺点——比如传输内容会被偷窥（嗅探）和篡改。
...
更多，参考[SSL与TLS的区别](http://kb.cnblogs.com/page/197396/)

## HTTPS
HTTP的安全版。即HTTP下加入SSL层，简单讲就是“HTTP 协议”和“SSL/TLS 协议”的组合。HTTPS的安全基础是SSL，因此加密的详细内容就需要SSL。

## SSL握手协议
SSL的体系结构中包含两个协议子层，其中底层是SSL纪录协议层（SSL Record Protocol Layer）；高层是SSL握手协议层。

SSL 协议既用到了公钥加密技术又用到了对称加密技术，对称加密技术虽然比公钥加密技术的速度快，可是公钥加密技术提供了更好的身份认证技术，公用密钥算法（通常为 RSA）是用来获得加密密钥交换和数字签名的。
SSL 的握手协议有效的让客户和服务器之间完成身份认证，其主要过程如下：

 ![SSL握手过程](/images/SSL握手.png)
...
[更多关于HTTPS](http://www.mahaixiang.cn/internet/1233.html)
